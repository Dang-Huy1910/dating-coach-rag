def test_message_rewrite_golden(client, stub_hits, monkeypatch):
    monkeypatch.setattr(
        ...
        lambda _prompt: '{"reply": "Tone đang ép. Nên mời cụ thể và để họ từ chối được.", "improved_draft": "Cuối tuần này cà phê 30 phút gần cầu giấy được không? Không sao nếu bạn bận.", "openers": null}',
    )
